  ...
  this.verify(req, function (err, verified) {
    if (err || !verified) {
      ...
      if (req.query.sid) {
        ...
        // avoid IPC
        if (self.clients[req.query.sid]) {
          debug('setting request to local worker');
          self.setRequest(req);
        } else {
          debug('proxying request to a different worker');
          self.proxyRequest(req);
        }
      } else {
        self.handshake(req.query.transport, req);
      }
    }
  });
};
...
/**
 * Assigns the request to a transport within this worker.
 *
 * @param {ServerRequest} request
 * @api private
 */

Server.prototype.setRequest = function (req) {
  this.clients[req.query.sid].transport.onRequest(req);
};

/**
 * Assigns the request to a remote worker object.
 *
 * @param {Worker} source worker
 * @param {String} request remote object id
 * @param {String} response remote object id
 * @api private
 */

Server.prototype.setProxiedRequest = function (worker, reqOid, resOid) {
  this.proxied[reqOid].res = this.proxied[resOid];
  this.setRequest(this.proxied[reqOid]);
};

/**
 * Sets up a proxy object in a remote worker.
 *
 * @param {String} worker id
 * @param {Object} local base object (source)
 * @param {Object} remote base object to create
 * @param {Array} fns to proxy. if `on` is included events are proxied
 * @param {Function} callback
 */

Server.prototype.proxy = function (worker, source, obj, methods, fn) {
  self.worker(worker, 'createProxy', obj, methods, function (oid) {
    self.sources[worker + '-' + oid] = source;

    if (~methods.indexOf('on')) {
      source.on = function (ev) {
        debug('captured remote event attached "%s"', ev);
        oldOn.call(req, ev, function () {
          self.worker(worker, 'proxyEvent', oid, ev, args);
        });
      }
    }
...
/**
 * Proxies a request to a different worker.
 *
 * @param {ServerRequest} request
 * @api private
 */

Server.prototype.proxyRequest = function (req) {
  var id = req.query.sid
    , self = this

  this.master('lookupWorker', id, function (worker) {
    debug('request belongs to worker %d', worker);

    self.proxy(worker, { headers: req.headers }, ['on'], function (oid) {
      self.proxy(worker, {}, ['writeHead', 'end'], function (oid2) {
        self.worker(worker, 'setProxiedRequest', oid, oid2);
      });
    });
  });
};
...
Server.prototype.handshake = function (transport, req) {
  ...
  // keep track of client in worker
  this.clients[id] = socket;
  this.clientsCount++;

  // keep track of sid in master
  if (this.inCluster) {
    this.master('handshake', id, done);
  } else {
    done();
  }
  ...
  function done () {
    ...
    socket.once('close', function () {
      // delete client from worker
      delete self.clients[id];
      self.clientsCount--;

      // delete client from master
      if (self.inCluster) {
        self.master('close', id);
      }
    });
  }
};
...
Server.prototype.handleUpgrade = function (req, socket, head) {
  var self = this;
  this.verify(this.prepare(req), function (err, verified) {
    if (err || !verified) {
      if (err) {
        debug('upgrade verification error');
      } else {
      ...
      self.ws.handleUpgrade(req, socket, head);
    }
  });
};
...
Server.prototype.onWebSocket = function (socket) {
  ...
  if (id) {
    if (this.clients[id]) {
      ...
      debug('proxying websocket to worker');
      this.proxyWebSocket(socket);
    }
  } else {
...
Server.prototype.setWebSocket = function (socket) {
...
Server.prototype.setProxiedWebSocket = function (oid) {
  this.setWebSocket(this.proxied[oid]);
};

/**
 * Proxies WebSocket to a different worker.
 *
 * @api private
 */

Server.prototype.proxyWebSocket = function (socket) {
  var id = req.query.sid;

  this.master('lookupWorker', id, function (worker) {
    debug('websocket belongs to worker %d', worker);

    self.proxy(worker, socket, {}, ['on', 'close', 'send'], function (oid) {
      self.worker(worker, 'setProxiedWebSocket', oid);
    });
  });
};
